...
\begin{equation}
\label{eq:MGC}
c^{kl}_{n} = \frac{1}{n^2 \sigma^{k}_{C} \sigma^{l}_{D}} \sum\limits_{i,j=1}^{n} \tilde{C}_{ij} \tilde{D}_{ij} I\big( r(C_{ij}) \leq k \big) I\big(  r(D_{ij}) \leq l  \big), \quad k= 1, \ldots, K_{\mathbf{U}}; l =1, \ldots, L_{\mathbf{L}},
\end{equation}
where $\sigma^{k}_{C}$ and $\sigma^{l}_{D}$ denote the standard deviations for truncated $\tilde{C}$ and truncated $\tilde{D}$ for each; $K_{\mathbf{U}} (\leq n)$ and $L_{\mathbf{X}} (\leq n)$ is the number of distinct values in data $\mathbf{U}$ and $\mathbf{X}$ respectively. Then we locate the optimal local correlation, i.e., optimal choice of included neighborhood $(k^{*}, l^{*})$, which yields \texttt{MGC} statistic. 
