...
Then $\texttt{dCorr}(\mathbf{U}, \mathbf{X}) \longrightarrow 0 \quad \mbox{ as } n \rightarrow \infty$ if and only if $\mathbf{U}$ is independent of $\mathbf{X}$. Therefore, both \texttt{dCorr} and \texttt{MGC} are consistent for testing dependence between $\mathbf{U}$ and $\mathbf{X}$.
	\label{theoremMain}
\end{theorem}
...
\end{equation} 
Namely, within-block edge probability is $0.5$, between-block edge probability is $0.2$ or $0.3$ depending on the communities. This 3-block model describes a nonlinear dependency between the network topology and the class labels, where \texttt{MGC} should work the best when coupled with a proper metric. A visualization of one sample graph is offered in Figure~\ref{fig:diffusions}. After repeatedly data generation and hypothesis testing by all methods, the powers are computed and shown in Figure~\ref{fig:threeSBM}, for which \texttt{MGC} combined with diffusion maps indeed yields the most superior power comparing to all other benchmarks.
...
\label{sec:conc}
	\vspace*{-0.2cm}
In this paper, we combined recent progress in dependency testing and metric learning into the graph domain, and showed that \texttt{MGC} on the diffusion distance offers an elegant and powerful solution to the network dependency problem, for which existing parametric methods lack. We proved that our method is consistent under almost all popular graph models; and empirically demonstrated it has superior power over all benchmarks, with \texttt{MGC} and the diffusion distance being the core elements behind the success.
...
\begin{proof}[\textbf{Proof of Lemma~\ref{main_lemma}}]
    ...
    &= \begin{pmatrix} \lambda^{t}_{1} \phi_{1}(i) & \lambda^{t}_{2} \phi_{2} (i)  & \cdots & \lambda^{t}_{q} \phi_{q}(i) \end{pmatrix} \in \mathbb{R}^{q},
	\end{align*}
	where $\{ \lambda_{j} \}$ and $\{ \phi_{j}  \}$ are the non-zero eigenvalues and corresponding eigenvectors of the transition matrix $\mathbf{P}$. Namely, $P \phi_{j} = \lambda_{j} \phi_{j}$ for $j \in \{1,2, ... , q \}$. If one can show that $P$ is always exchangeable, i.e., $P_{\sigma(i) \sigma(j)}$ always distributes the same as $P_{ij}$ for $i,j=1,\ldots,n$, then its eigenvalues and eigenvectors are also exchangeable, so are the diffusion maps.
	
		Given joint exchangeability of the graph $\mathbf{G}$, i.e. $(A_{ij}) \stackrel{d}{=} \big( A_{\sigma(i) \sigma(j)} \big)$. 
	As $A_{ij}$ is unweighted (binary), we have 
		\begin{align*}
	P &= A_{ij} / \sum\limits_{j} A_{ij} \\
	  &= A_{ij} /  (1 + \sum\limits_{l \neq j} A_{il}),
	\end{align*}
	\cs{why $A_{ij}=1$ above?}
	Moreover, $A_{ij}$ and $(1 + \sum\limits_{l \neq j} A_{il})$ are conditionally independent, so are $A_{\sigma(i) \sigma(j)}$ and $(1 + \sum\limits_{l \neq j} A_{\sigma(i) \sigma(l)})$, based on the \textit{Aldous-Hoover Theorem} \cite{aldous1981representations}. \cs{why conditional \textit{i.i.d.} leads to the following equation? Some statement is missing here.}
    Then for $i \neq j$, we have the following for $P$:
	\begin{equation}
		\vspace*{-0.2cm}
	\big( P_{ij} \big) = \left(  \frac{A_{ij}}{1 - A_{ij} + \sum\limits_{j=1}^{n} A_{ij} } \right)  \sim \left( \frac{A_{\sigma(i) \sigma(j)} }{1 - A_{\sigma(i) \sigma(j)} + \sum\limits_{\sigma(j) = 1}^{n} A_{\sigma(i) \sigma(j)} } \right) = \big( P_{\sigma(i) \sigma(j)} \big),
	\end{equation}
	where $\sim$ means same distribution;
	and for $i = j$, $P_{ij} = P_{\sigma(i) \sigma(j)} = 0$. Therefore $P$ is always exchangeable given an exchangeable graph $\mathbf{G}$. 
	
	Therefore, the diffusion maps are exchangeable, and also conditional \textit{i.i.d.} asymptotically.
	\cs{Again, I don't see why connected-ness matter in the proof; the weight only affects the $1$ in the $\sum\limits_{l \neq j} A_{il}$ equation, right? And directed do not matter either.}
\end{proof}
