...
Coifman and Lafon~\cite{coifman2006diffusion,lafon2006diffusion} proposed multiscale geometries of data called diffusion maps, which are constructed by iterating the transition matrix that determines random walk on graph. 
\cs{do they really proposed diffusion maps on graphs by transition matrix? from wikipedia they proposed diffusion maps on any kernel/similarity matrix, but not this specific form. Or is it from Minh's work? if not from coifman, break the above sentence, say they proposed it on any similarity matrix, and we (or cite minh) tailored to the transition matrix. }
Given the $n \times n$ adjacency matrix $\mathbf{A}$, the $n \times n$ transition matrix of $\mathbf{P}$ is defined by $P_{ij} = A_{ij} /  \sum\limits_{j=1}^{n} A_{ij}$, indicating the probability of moving forward from node $i$ to node $j$ for $i,j = 1, \ldots , n$. The diffusion maps at time $t$ are computed as follows : 
	\begin{align*}
...
\begin{proof}[\textbf{Proof of Lemma~\ref{main_lemma}}]
    ...
    &= \begin{pmatrix} \lambda^{t}_{1} \phi_{1}(i) & \lambda^{t}_{2} \phi_{2} (i)  & \cdots & \lambda^{t}_{q} \phi_{q}(i) \end{pmatrix}^{T} \in \mathbb{R}^{n \times q},
	\end{align*}
	where $\{ \lambda_{j} \}$ and $\{ \phi_{j}  \}$ are the non-zero eigenvalues and corresponding eigenvectors of the transition matrix $\mathbf{P}$. 
    
		Denote any permutation matrix as $\pi$, it suffices to show $\mathbf{U}$ distributes the same as $\mathbf{U}\pi$. By basic matrix manipulation of the eigenvectors, it is equivalent to show $\mathbf{P}$ distributes the same as $\pi \mathbf{P}\pi$. 
		
		By the definition of an exchangeable graph, $A_{\sigma(i)\sigma(j)}$ distributes the same $A_{ij}$. It follows that
				\begin{align*}
