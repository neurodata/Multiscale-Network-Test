...
\begin{proof}[\textbf{Proof of Lemma~\ref{main_lemma}}]
	...
	\begin{align*}
	\mathbf{U}(i) &=\{ \mathbf{u}(1) , \ldots, \mathbf{u}(n) \}\\
    &= \begin{pmatrix} \lambda^{t}_{1} \phi_{1}(i) & \lambda^{t}_{2} \phi_{2} (i)  & \cdots & \lambda^{t}_{q} \phi_{q}(i) \end{pmatrix} \in \mathbb{R}^{q},
	\end{align*}
	where $\{ \lambda_{j} \}$ and $\{ \phi_{j}  \}$ are the non-zero eigenvalues and corresponding eigenvectors of the transition matrix $\mathbf{P}$. Namely, $P \phi_{j} = \lambda_{j} \phi_{j}$ for $j \in \{1,2, ... , q \}$. If one can show that $\mathbf{P}$ is always exchangeable, i.e., $\mathbf{P}_{\sigma(i) \sigma(j)}$ always distributes the same as $\mathbf{P}_{ij}$ for $i,j=1,\ldots,n$, then its eigenvalues and eigenvectors are also exchangeable, so are the diffusion maps.
	
		Given joint exchangeability of the graph $\mathbf{G}$, we have $(A_{ij}) \stackrel{d}{=} \big( A_{\sigma(i) \sigma(j)} \big)$, where $\stackrel{d}{=}$ stands for same distribution. 
	It follows that
		\begin{align*}
	\mathbf{P}_{\sigma(i) \sigma(j)} &= A_{\sigma(i) \sigma(j)} / \sum\limits_{j} A_{\sigma(i)j} \\
	  &\stackrel{d}{=} A_{ij} /  \sum\limits_{j} A_{ij} \\
		&= \mathbf{P}_{ij},
	\end{align*}
	as both the numerator and the denominator have the same distribution and have the same dependent structure, before or after the permutation. Therefore $P$ is always exchangeable given an exchangeable graph $\mathbf{G}$.
