...
In this section, we introduce a family of network geometries of an exchangeable graph \cite{coifman2006diffusion} that can yield node-wise conditional \textit{i.i.d.} samples.

A graph $\mathbf{G}$ is called exchangeable if and only if its adjacency matrix $\mathbf{A}$ is jointly exchangeable \cite{orbanz2015bayesian}, i.e.~for every permutation $\sigma$ of $n$, $(A_{ij}) \stackrel{d}{=} (A_{\sigma(i) \sigma(j)})$. Most generative statistical network models, such as stochastic block model~\citep{rohe2011spectral, todeschini2016exchangeable}, degree-corrected stochastic block model~\citep{todeschini2016exchangeable}, and latent position model~\citep{sussman2014consistent}, satisfy this property.
...
\end{lemma}
\cs{removed undirected and unweighted for now; I still need to change proof accordingly} : \textcolor{red}{I haven't tried to prove in this case. If you want to try undirected graph, you need different definitions on diffusion maps, and i think unweighted graph needs different proof.}

\cs{add in a lemma saying all models above are exchangeable} \textcolor{red}{I added references in the above.}
...
\label{sec:simulation}
	\vspace*{-0.2cm}
Next we investigate our approach via simulated models and empirical performances. In the simulation studies, we compare the empirical testing powers of four test statistics: \texttt{MGC}, \texttt{dCorr}, Heller-Heller-Gorfine (\texttt{HHG}), and likelihood ratio test of Fosdick and Hoff (\texttt{FH})~\cite{fosdick2015testing}. For the first three statistics, we further experiment on three different metrics of the network topology: the Euclidean distances of the diffusion maps (\texttt{DF}), of each column of adjacency matrix (\texttt{AM}), and of the latent factors (\texttt{LF}, which is based on singular value decomposition of the adjacency matrix). The \texttt{FH} likelihood ratio test must always be based on the latent factors.

For each simulation model and each test, we repeatedly generate sample data for $500$ times, carry out the permutation test, and reject the null if the resulting p-value is less than $0.05$. The testing power of each method equals the percentage of correct rejection. %The simulation models are shown by joint distribution of adjacent matrix $\mathbf{A}$ and nodal attributes $\mathbf{X}$. %We introduced a popular network model of Stochastic Block Model (SBM) as our main simulated networks. 
...
\cs{I will say $t=5$ here, and add $t=3$ to $10$ is very similar.}

\begin{SCfigure}[][ht]
	\centering
	\includegraphics[width=0.4\paperwidth, height=0.4\paperwidth]{ThreeSBM_results_short.png}
	\caption{This power heatmap illustrates the superior power of multiscale generalized correlation (\texttt{MGC}) with diffusion distance in the three-block SBM (equation~\ref{eq:Three}), compared to all other choices of distance metric and all other test statistics.}
	\label{fig:threeSBM}
\end{SCfigure}
...
\begin{figure}[ht]
	...
	\caption{This demonstrates how the \texttt{MGC} statistic can better capture the nonlinear dependencies than other statistics. X-axis of $\theta$ controls the existence/amount of nonlinear dependency, where strong nonlinearity exists when $\theta > 0.2$ and gets stronger as $\theta$ increases. You can see the discrepancy in power between global and local scale tests also gets larger accordingly, mostly due to decreasing power of \texttt{dCorr} or \texttt{FH} test but relatively stable power of \texttt{MGC} under nonlinear dependency.}
	\label{fig:powerplot}
\end{figure}
...
\begin{figure}[ht]
	...
	\vspace*{-0.5cm}
\end{figure}	
\cs{it is a bit confusing figure 3 \& 4 has different legends. Let us do MGC $\circ$ DM, MGC $\circ$ AM, MGC $\circ$ LF for the legends.}
