...
\begin{figure}[ht]
	...
	\caption{This demonstrates how the \texttt{MGC} statistic can better capture the nonlinear dependencies than other statistics. X-axis of $\theta$ controls the existence/amount of nonlinear dependency, where strong nonlinearity exists when $\theta > 0.2$ and gets stronger as $\theta$ increases. You can see the discrepancy in power between global and local scale tests also gets larger accordingly, mostly due to decreasing power of \texttt{mCorr} or \texttt{FH} test but relatively stable power of \texttt{MGC} under nonlinear dependency.}
	\label{fig:powerplot}
\end{figure}
