...
\subsubsection{[EG1]}
\begin{itemize}
	\item {\it  [EG1] introduce an exchangeable graph \/}
\end{itemize}
...
To talk about independence of $U$, we need more general concept of exchangeable graph. 
...
\subsubsection{[EG2]}
\begin{itemize}
	\item {\it  [EG2] Derive exchangeability and iid representation of A \/}
\end{itemize}
...
\subsubsection{[EG3]}
\begin{itemize}
	\item {\it  [EG3] examples of exchangeable graphs \/}
\end{itemize}
...
There have been a lot of efforts to represent the network in terms of a summarizing network factor( [Fosdick $\&$ Hoff (2015)] ) or some meaningful coefficients, e.g. centrality or connectivity. However, there has been no vertex-wise variable which provides a configuration of vertex over network space without losing any information. [Coifman $\&$ Lafon] demonstrated that diffusion maps provide a meaningful multiscale geometries of data while keeping information on every local relation. Diffusion map is constructed via Markov chain on graph. Here an adjacency matrix $\boldsymbol{A}$ acts as a kernel, representing a similarity between each node in $\boldsymbol{G}$. The adjacency matrix also takes into account every single relationship between nodes, rather than estimating or summarizing network structures.
...
However, \texttt{dCorr} and even \texttt{MCorr} still perform not very well in various non-linear settings and under existence of outliers ( [Cencheng et al. (2016)]). Out of this concern, Cencheng at al. developed Multiscale Generalized Correlation (\texttt{MGC}) by adding local scale on correlation coefficients. 
...
\begin{thebibliography}{1}
	...
	\bibitem{Hoff} Hoff, P. D., Raftery, A. E., $\&$ Handcock, M. S. (2002). Latent space approaches to social network analysis. Journal of the american Statistical association, 97(460), 1090-1098.
	
	
	\bibitem{Austin} Austin, A., Linkletter, C., $\&$ Wu, Z. (2013). Covariate-defined latent space random effects model. Social Networks, 35(3), 338-346.
	
	\bibitem{Lazarsfeld} Lazarsfeld, P. F, $\&$ Henry, N. W. (1968). Latent structure analysis. New York: Houghton, Mifflin.
	
	\bibitem{Fosdick} Fosdick, B. K., $\&$ Hoff, P. D. (2015). Testing and modeling dependencies between a network and nodal attributes. Journal of the American Statistical Association, 110(511), 1047-1056.
