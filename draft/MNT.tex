...
\begin{itemize}	
	...
	\begin{equation}
		...
		& \mathbf{X}_{i} | Z_{i} \overset{i.i.d}{\sim} f_{\mathbf{X}|Z}(\mathbf{x}|z) \stackrel{d}{=} (0.4+0.2z) \mathbf{N}_{1} + (0.6 - 0.2z) \mathbf{N}_{2} \\
		& A_{ij} | Z_{i}, Z_{j} \overset{i.i.d}{\sim} f_{A|Z} (a_{ij} | z_{i}, z_{j}) \stackrel{d}{=} Bern(0.4) I\big( |z_{i} - z{j}| = 0 \big) + Bern(0.1 ) I(|z_{i} - z_{j}| > 0), \\
		& \quad \quad \quad  i < j, i,j =1,\ldots, n
		\end{split}
		\end{gathered}
	\end{equation}
	where $\mathbf{N}_{1} = $ and $\mathbf{N}_{2} = $
