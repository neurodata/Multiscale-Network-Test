...
\begin{figure}[H]
	...
	\caption{Population probability distribution and realized values of $\mathbf{A}$ and $\mathbf{X}$ (scaled by 1/3) in the left two panels. Euclidean distance applied to realized $\mathbf{A}$ and $\mathbf{X}$ are presented in the right two panels. Data generation follows Eq.~\ref{eq:Three}.}
	\label{fig:matrics}
\end{figure}	
...
\begin{figure}[H]
	...
	\includegraphics[width=2in]{../Figure/tildeC.png}
	\includegraphics[width=2in]{../Figure/tildeD.png}
	\includegraphics[width=2in]{../Figure/tildeCD.png}
...
Lemma~\ref{lemma1} and its following Lemma~\ref{lemma2} facilitates the use of distance correlation through satisfying \textit{Theorem 2} in \cite{szekely2007measuring}.  


\begin{theorem}
	\label{theoremMain}
	Suppose that $(\mathbf{X},\mathbf{Y})=\{(\mathbf{x}_{j},\mathbf{y}_{j}), j=1,2,...,n\}$ is a sequence of exchangeable random variables, where $(\mathbf{x}_{j},\mathbf{y}_{j})$ is identically distributed as $(\mathbf{x},\mathbf{y})$ with finite moments. Then $\mathcal{V}^{2}_{n}(\mathbf{X},\mathbf{Y}) \rightarrow 0$ if and only if $x$ is independent of $y$. Moreover, distance correlation and MGC are consistent for testing dependence between $\mathbf{X}$ and $\mathbf{Y}$, i.e., the testing power converges to $1$ asymptotically for any dependency of finite moments.
\end{theorem}

\begin{theorem}
	\label{theorem2}
	MGC is consistent in network (graph0 dependence testing, for any graph metric such that $(\mathbf{X},\mathbf{Y})$ is exchangeable and of finite moments. In particular, the consistency holds for the diffusion maps, adjacency vectors, and the estimated latent positions.
\end{theorem}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{proof}[\textbf{Proof of Lemma~\ref{lemma2}} Convergence of empirical characteristic function of exchangeable variables]
...
\begin{proof}[\textbf{Proof of Lemma~\ref{lemma1}} Empirical characteristic function of exchangeable variables]
